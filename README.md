...
Or run the same arena on MuJoCo instead (see **Simulation backends** below):
...
The same arena, robot and ROS interface run on either physics engine. Pick with
the launch file:

| | Gazebo Harmonic | MuJoCo |
|---|---|---|
| Launch | `erc_bringup simulation.launch.py` | `erc_bringup mujoco_simulation.launch.py` |
| World | `erc_description/worlds/erc_world.sdf` | generated by `generate_mujoco_world.py` |
| Base drive | `gz-sim-mecanum-drive-system` | `mujoco_ros2_control_plugins/BaseVelocityPlugin` |
| ros2_control | `gz_ros2_control/GazeboSimSystem` | `mujoco_ros2_control/MujocoSystemInterface` |

Both present the same controllers, `/cmd_vel`, `/odom`, `/joint_states`, and the
same head-camera topic names, and both honour `ERC_SEED` for the book and
number-marker layout.

**Not yet reproduced on MuJoCo:** `/scan_front_raw`, `/scan_rear_raw`,
`/base_imu`, `/contacts` and `/bin_contacts`. The LiDAR and IMU are reachable
(`mujoco_ros2_control` ships a 3D-lidar extension, and IMUs are exposed as
...
|---|---|---|
| `/scan_front_raw` | `sensor_msgs/msg/LaserScan` | *(Gazebo only)*  Front LiDAR |
| `/scan_rear_raw` | `sensor_msgs/msg/LaserScan` | *(Gazebo only)*  Rear LiDAR |
...
| ROS 2 | Humble |
| Gazebo | Harmonic |
| MuJoCo | via `mujoco_ros2_control` (vendored) |
| Physics engine | DART (Gazebo) / MuJoCo |
| DDS | CycloneDDS |
| Robot description | PAL Robotics (vendored, see above) |
| Controller framework | ros2_control + `gz_ros2_control` / `mujoco_ros2_control` |
...
The MuJoCo backend derives its own URDF from that same file at launch (`erc_bringup/scripts/generate_mujoco_urdf.py`), so both backends run a robot byte-derived from the one the competition ships. PAL's `tiago_pro.urdf.xacro` has no `sim_type` argument, so there is no upstream MuJoCo path to call into.
