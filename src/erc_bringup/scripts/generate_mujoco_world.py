...
MARKER_HALF = (0.15, 0.001, 0.15)    # the SDF's 0.3 x 0.002 x 0.3 plate
...
# Yaw the marker plate 90 degrees so its printed face points at the robot.
MARKER_QUAT = '0.70710678 0 0 0.70710678'
...
def build_world(share, seed=None):
    ...
    # ── Static arena shell ──
    body = [
        '    <light name="arena_light_top" pos="1 0 4" dir="0 0 -1" directional="true"/>',
        '    <light name="arena_light_front" pos="1 -6 3" dir="0 1 -0.5" directional="true"/>',
        '',
    ...
    for col in range(NUM_COLUMNS):
        ...
        y = SHELF_Y + COLUMN_Y_OFFSETS[col]
        body.append(
            f'    <geom name="number_marker_col_{col + 1}" class="arena_visual" type="box" '
            f'pos="{NUMBER_MARKER_PLATE_X:g} {y:g} {NUMBER_MARKER_PLATE_Z:g}" '
            f'quat="{MARKER_QUAT}" size="{fmt(MARKER_HALF)}" material="marker_{number}"/>')
